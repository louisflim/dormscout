...
export default function Login({ setUserType }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (result.success) {
      // Read the real userType from the account that was just logged in
      ...
      // Guard: wrong login page for this account type
      if (realUserType && realUserType !== urlUserType) {
        setError(
      ...
      localStorage.setItem('userType', realUserType || urlUserType);

      if (setUserType) {
        setUserType(realUserType || urlUserType);
      }

      // ✅ FIX: correct entry route — /dashboard does not exist in the router
      navigate('/overview');
    } else {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Look for the dorms{' '}
          <span style={{ color: SECONDARY, fontStyle: 'italic' }}>you </span>
          <span style={{ color: PRIMARY, fontStyle: 'italic' }}>deserve.</span>
        </p>
      </div>
        ...
        <div style={{ maxWidth: '400px', width: '100%' }}>
          <div
            style={{
              ...
              padding: '24px',
              boxShadow:
                '0 2px 8px rgba(0,0,0,0.1), 0 0 1px rgba(0,0,0,0.1)',
            }}
          >
            <h2
              style={{
